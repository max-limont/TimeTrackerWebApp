...
import {createCalendarDay, editCalendarDay, removeCalendarDay} from "../../../store/actions/calendar/calendarActions";
                ...
                <form onSubmit={event => submit(event)}>
                    <div className={"form-group"}>
                        <div className={"form-item w-100"}>
                            <label>Name</label>
                            <input value={state.data?.title ?? ''} onChange={event => setState({...state, data: {...state.data, title: event.target.value} as CalendarDayType})} />
                        </div>
                        <div className={"form-item w-100"}>
                            <label>Day type</label>
                            <select value={state.data?.dayTypeId ? state.data.dayTypeId : 0} onChange={event => setState({...state, data: {...state.data, dayTypeId: parseInt(event.target.value)} as CalendarDayType})}>
                        ...
                        <div className={"form-item w-100"}>
                            ...
                            <Switch width={40} height={18} handleDiameter={18} onChange={() => setState({...state, isDailyRange: !state.isDailyRange, data: {...state.data, endDate: !state.isDailyRange ? new Date() : data?.endDate} as CalendarDayType})} checked={state.isDailyRange} checkedIcon={false} uncheckedIcon={false} />
                        </div>
                        <div className={"form-item w-100"}>
                            <label>Date</label>
                            <input type={"date"} defaultValue={convertDateToFormat(state.data?.date ?? new Date())} onChange={event => setState({...state, data: {...state.data, date: new Date(event.target.value)} as CalendarDayType})} />
                        </div>
                        { state.isDailyRange &&
                            <div className={"form-item w-100"}>
                                <label>Last date in range</label>
                                <input type={"date"} defaultValue={convertDateToFormat(state.data?.endDate ?? new Date())} onChange={event => setState({...state, data: {...state.data, endDate: new Date(event.target.value)} as CalendarDayType})} />
                            </div>
                        }
                    </div>
