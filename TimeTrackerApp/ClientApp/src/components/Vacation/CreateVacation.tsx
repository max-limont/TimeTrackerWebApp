    ...
    return (
        <div className={`form-event-container dark-background ${!visible && "hidden"}`}>
            <div className={"form-event"}>
                ...
                <form onSubmit={(e) => onFinish(e)} onChange={()=>{
                    setError("");
                }}>
                    ...
                    <div className={"form-group"}>
                        <div className={"form-item w-100"}>
                            ...
                            <input className={validationError != "" ? "validation-error color-red" : ""} value={startingTime}
                                   type="date"
                                   onChange={(e) => setVacation({...vacation, startingTime: e.target.value})}/>
                        </div>
                        <div className={"form-item w-100"}>
                            <label>Ending Time</label>
                            <input className={validationError != "" ? "validation-error color-red" : ""} value={endingTime}
                                   type="date"
                                   onChange={event => setVacation({...vacation, endingTime: event.target.value})}/>
                        </div>
                        <div className={"form-item w-100"}>
                            <label>Comment</label>
                            <textarea className="text-area-form" defaultValue={comment}
                                      onChange={event => setVacation({...vacation, comment: event.target.value})}/>
                        </div>
                    </div>
