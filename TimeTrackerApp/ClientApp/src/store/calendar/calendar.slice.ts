...
import {CalendarTypes, CreateCalendarDayType, DateInputType, DayTypes} from "../../types/calendar.types";
...
export const calendarSlice = createSlice({
    ...
    reducers: {
        ...
        createDay: (state, action: PayloadAction<CalendarTypes>) => {
        ...
        editDay: (state, action: PayloadAction<CalendarTypes>) => {
...
export const editCalendarDay = createAction<CalendarTypes>("EditCalendarDay");
export const removeCalendarDay = createAction<number>("RemoveCalendarDay");

export const getMonthCalendarDays = (date: Date, calendarDaysData: CalendarTypes[]): CalendarDay[] => {
    ...
    for (let day: Date = startDate; day <= endDate; day = new Date(new Date(day).setDate(new Date(day).getDate() + 1))) {
        ...
        const calendarDay: CalendarDay = {
            date: day,
            data: day.getDay() % 6 == 0 ? [{ title: 'Day off', date: day, dayTypeId: DayTypes.Weekend.valueOf() } as CalendarTypes, ...dayData] : dayData
        }
